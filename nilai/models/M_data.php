...
class M_data extends CI_Model{
	function getNilai(){
		return $this->db->query("SELECT * FROM nilai_siswa n JOIN kategori_nilai k ON n.kategori_nilai_id=k.id_kategorinilai JOIN jenis_nilai_akhir j ON n.jenis_na_id=j.id_jenis_na JOIN mapelkkm m ON n.mapel_id=m.id_mapel JOIN mapel mp ON m.id_namamapel=mp.id_namamapel JOIN siswa s ON n.NISN=s.nisn");
	}
	function getNilaiKelasMapel($id_kelas_reguler_berjalan, $id_mapel){
		return $this->db->query("SELECT * FROM nilai_siswa n JOIN kategori_nilai k ON n.kategori_nilai_id=k.id_kategorinilai JOIN jenis_nilai_akhir j ON n.jenis_na_id=j.id_jenis_na JOIN mapelkkm m ON n.mapel_id=m.id_mapel JOIN mapel mp ON m.id_namamapel=mp.id_namamapel JOIN siswa s ON n.NISN=s.nisn JOIN siswa_kelas_reguler_berjalan sk ON s.NISN = sk.NISN WHERE m.id_mapel = '$id_mapel' AND sk.id_kelas_reguler_berjalan = '$id_kelas_reguler_berjalan'")->result();
	}
	...
	function getNilaiSiswa($where = array()) {
		...
		$this->db->join('jenis_nilai_akhir', 'nilai_siswa.jenis_na_id = jenis_nilai_akhir.id_jenis_na');
		$this->db->where($where);
		return $this->db->get("nilai_siswa")->row();
	}
